...
class TestMessagesManager(DaoTest):
    faker = Faker()

    @classmethod
    def setUpClass(cls):
        super(TestMessagesManager, cls).setUpClass()
        from tests.models.test_messages import TestMessages
        cls.test_message = TestMessages
    ...
    def test_crud(self):
        for _ in range(0, 10):
            ...
            #READ MESSAGE TEST
            message1 = self.message_manager.get_message_by_id(message.id)
    ...
    def test_actions(self):
        ...
        message = self.test_message.generate_random_message()
        self.message_manager.create_message(message=message)
        self.message_manager.report_message_by_id(message.id)
        message1 = self.message_manager.get_message_by_id(message.id)
